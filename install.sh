...
main() {
    ...
    # MCP config (copy template if doesn't exist)
    if [ -f "$CONFIG_DIR/config-templates/mcp.json.template" ]; then
        if ! [ -f ~/.claude/mcp.json ]; then
            if $DRY_RUN; then
                dry_run_msg "Would create mcp.json from template"
            else
                sed "s|\${HOME}|$HOME|g" "$CONFIG_DIR/config-templates/mcp.json.template" > ~/.claude/mcp.json
                echo -e "${GREEN}✓${RESET} mcp.json (created from template)"
                has_changes=true
            fi
        else
            echo -e "${YELLOW}○${RESET} mcp.json (exists, not overwriting)"
        fi
    fi
